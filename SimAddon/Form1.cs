...
namespace SimAddon
{
    ...
    public partial class Form1 : Form
    {
        ...
        /// </summary>
        private async Task CheckReservationAsync()
        {
            try
            {
                // Try to get callsign from FlightRecorder plugin Settings or control
                string callsign = string.Empty;
                var frPlugin = plugsMgr.plugins.FirstOrDefault(p => string.Equals(p.getName(), "FlightRecorder", StringComparison.OrdinalIgnoreCase));
                if (frPlugin != null)
                {
                    try
                    {
                        // Try plugin Settings class (e.g. FlightRecPlugin.Properties.Settings.Default.callsign)
                        var asm = frPlugin.GetType().Assembly;
                        var settingsType = asm.GetTypes().FirstOrDefault(t => t.Name == "Settings" && t.Namespace != null && t.Namespace.EndsWith(".Properties"));
                        if (settingsType != null)
                        {
                            var defaultProp = settingsType.GetProperty("Default", BindingFlags.Static | BindingFlags.Public);
                            if (defaultProp != null)
                            {
                                var settingsInstance = defaultProp.GetValue(null);
                                if (settingsInstance != null)
                                {
                                    var csProp = settingsType.GetProperty("callsign", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                                    if (csProp != null)
                                    {
                                        callsign = csProp.GetValue(settingsInstance) as string ?? string.Empty;
                                    }
                                }
                            }
                        }
                    ...
                    {
                        Logger.WriteLine("CheckReservation: error reading plugin settings: " + ex.Message);
                    }

                    // fallback to tbCallsign control if still empty
                    if (string.IsNullOrWhiteSpace(callsign))
                    {
                        try
                        {
                            var tbField = frPlugin.GetType().GetField("tbCallsign", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
                            if (tbField != null)
                            {
                                var tb = tbField.GetValue(frPlugin) as TextBox;
                                if (tb != null)
                                {
                                    callsign = tb.Text?.Trim() ?? string.Empty;
                                }
                            }
                        }
                        catch { }
                ...
                // fallback to main settings if empty
                if (string.IsNullOrWhiteSpace(callsign))
                {
                    try
                    {
                        // Some projects expose callsign in their own Settings; try main Properties.Settings if available
                        var mainSettings = typeof(Properties.Settings).GetProperty("Default", BindingFlags.Static | BindingFlags.Public)?.GetValue(null);
                        if (mainSettings != null)
                        {
                            var csProp = mainSettings.GetType().GetProperty("callsign", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                            if (csProp != null)
                            {
                                var val = csProp.GetValue(mainSettings) as string;
                                callsign = val?.Trim() ?? string.Empty;
                            }
                        }
                    }
                    catch { /* ignore */ }
                ...
                Logger.WriteLine("CheckReservation: callsign to check='" + callsign + "'");
                ...
                string baseUrl = Properties.Settings.Default.GSheetAPIUrl ?? string.Empty;
                if (string.IsNullOrWhiteSpace(baseUrl))
                {
                    Logger.WriteLine("CheckReservation: no GSheetAPIUrl configured, skipping.");
                    return;
                }

                string apiUrl = baseUrl.TrimEnd('/') + "/api/api_check_reservation.php?callsign=" + Uri.EscapeDataString(callsign);
                Logger.WriteLine("CheckReservation: calling " + apiUrl);

                using var http = new HttpClient { Timeout = TimeSpan.FromSeconds(8) };
                HttpResponseMessage resp = await http.GetAsync(apiUrl);
                ...
                string immat = string.Empty;
                string departure = string.Empty;
                string arrival = string.Empty; // arrival variable
                ...
                try
                {
                    ...
                    if (root.ValueKind == JsonValueKind.Object)
                    {
                        ...
                        {
                            if (r.ValueKind == JsonValueKind.True) reserved = true;
                            else if (r.ValueKind == JsonValueKind.False) reserved = false;
                            else if (r.ValueKind == JsonValueKind.Number) reserved = r.GetInt32() != 0;
                        }

                        if (root.TryGetProperty("reservation", out var res) && res.ValueKind == JsonValueKind.Object)
                        {
                            var reservation = res;
                            if (reservation.TryGetProperty("message", out var m) && m.ValueKind == JsonValueKind.String) message = m.GetString() ?? string.Empty;
                            if (reservation.TryGetProperty("immat", out var im) && im.ValueKind == JsonValueKind.String) immat = im.GetString() ?? string.Empty;
                            if (reservation.TryGetProperty("icao_dep", out var id) && id.ValueKind == JsonValueKind.String) departure = id.GetString() ?? string.Empty;
                            if (reservation.TryGetProperty("icao_arr", out var ia) && ia.ValueKind == JsonValueKind.String) arrival = ia.GetString() ?? string.Empty;
                        }
                        else
                        {
                            // fallback: maybe message at top
                            if (root.TryGetProperty("message", out var m2) && m2.ValueKind == JsonValueKind.String) message = m2.GetString() ?? string.Empty;
                        }
                    }
                    else
                    {
                        reserved = !string.IsNullOrWhiteSpace(body);
                        message = body.Trim();
                    }
                }
                catch (JsonException)
